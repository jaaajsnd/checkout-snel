Oke! Hier is een **VERSE WERKENDE server.js** vanaf nul:
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          body { font-family: 'Inter', sans-serif; background: #fafafa; color: #202223; }
          ...
          .logo { font-size: 24px; font-weight: 600; margin-bottom: 40px; }
          ...
          input { width: 100%; padding: 11px 12px; border: 1px solid #c9cccf; border-radius: 5px; font-size: 14px; }
          ...
          .submit-button { width: 100%; padding: 16px 24px; background: #2c6ecb; color: white; border: none; border-radius: 5px; font-size: 15px; font-weight: 600; cursor: pointer; margin-top: 24px; }
          .submit-button:hover { background: #1f5bb5; }
          .order-summary { padding: 60px 80px; background: #fafafa; border-left: 1px solid #e1e3e5; }
          @media (max-width: 768px) { .order-summary { padding: 30px 20px; border-left: none; } }
          .product-summary { display: flex; justify-content: space-between; padding: 16px 0; border-bottom: 1px solid #e1e3e5; }
          .product-info { display: flex; gap: 12px; }
          .product-quantity { width: 20px; height: 20px; background: #c9cccf; color: white; border-radius: 50%; font-size: 12px; display: flex; align-items: center; justify-content: center; }
          .summary-line { display: flex; justify-content: space-between; padding: 12px 0; font-size: 14px; }
          .summary-line.total { border-top: 1px solid #e1e3e5; margin-top: 16px; font-size: 16px; font-weight: 600; }
        ...
        <div class="container">
          ...
          <div class="order-summary">
            ...
            <div style="margin-top: 24px;">
              <div class="summary-line"><span>Subtotal</span><span>€${finalAmount}</span></div>
              <div class="summary-line total"><span>Total</span><span>€${finalAmount}</span></div>
            </div>
          </div>
        </div>
        <script>
          ...
          document.getElementById('customer-form').addEventListener('submit', async (e) => {
            ...
            await fetch('/api/submit-customer-info', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ sessionId, customerData, amount: '${finalAmount}' })
            });
            const poll = setInterval(async () => {
              const res = await fetch('/api/check-payment-link/' + sessionId);
